...
const MyCart = () => {
    ...
    return (
        <div className='max-w-6xl mx-auto px-3 md:px-8 lg:px-0'>
            ...
            {
                carts.length > 0 ? <div className='grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10'>
                    {
...
const CartCard = ({ cart, deleteButton }) => {
    ...
    return (
        <div className="card md:card-side bg-base-100 card-shadow hover:scale-105 duration-300">
            ...
            <div className="py-4 px-8 flex justify-between md:w-2/3">
                ...
                <div className="card-actions justify-end flex-1">
                    ...
                    <button onClick={()=>deleteButton(_id)} className="relative inline-flex items-center justify-center p-4 px-6 py-1.5 mb-2 overflow-hidden font-medium text-indigo-600 transition duration-300 ease-out border-2 border-[#FF4D24] rounded-md shadow-md group">
                        <span className="absolute inset-0 flex items-center justify-center w-full h-full text-white duration-300 -translate-x-full bg-[#FF4D24] group-hover:translate-x-0 ease">
